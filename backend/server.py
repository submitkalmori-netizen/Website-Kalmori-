...
@api_router.get("/label/dashboard")
async def label_dashboard(request: Request):
    """Get label-level overview: total artists, streams, revenue, releases"""
    user = await get_current_user(request)

    # Get all artists under this label
...
@api_router.get("/label/artists")
async def label_get_artists(request: Request):
    """Get all artists under this label with their stats"""
    user = await get_current_user(request)
...
@api_router.post("/label/artists/invite")
async def label_invite_artist(data: InviteArtistInput, request: Request):
    """Add an artist to the label roster by email"""
    user = await get_current_user(request)
...
@api_router.delete("/label/artists/{artist_id}")
